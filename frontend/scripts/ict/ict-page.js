...
async function renderICTPage() {
  ...
  try {
    if (page === 'ict-dashboard.html' || page === 'ict-health.html' || page === 'ict-activity.html' || page === 'ict-security.html') {
      ...
      content.innerHTML = `<section class="ict-page-panel"><h2>Email &amp; Notifications</h2><p>Send a message through the configured SMTP service.</p><div id="ictSmtpStatus" class="ict-status-card">Checking SMTP connection...</div><form class="ict-mail-form" id="ictStandaloneEmail"><label>Recipients<textarea name="to" rows="3" placeholder="name@example.com, another@example.com" required></textarea></label><label>Subject<input name="subject" required></label><label>Message<textarea name="text" rows="8" required></textarea></label><button type="submit">Send email</button><span id="ictStandaloneEmailStatus" class="ict-mail-status"></span></form></section>`;
      try { const smtp = await ictPageRequest('email/status'); document.getElementById('ictSmtpStatus').innerHTML = `<span>SMTP service</span><strong>${ictPageEscape(smtp.email.reachable ? 'ONLINE' : smtp.email.configured ? 'WARNING' : 'NOT CONFIGURED')}</strong>`; } catch (error) { document.getElementById('ictSmtpStatus').textContent = 'SMTP status unavailable'; }
